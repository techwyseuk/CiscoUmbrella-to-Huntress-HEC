# Cisco Umbrella MSP Portal to Huntress HEC Log Shipping Script
# High-performance script with concurrent processing for PowerShell 7+

param(
    [string]$ApiKey = "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",
    [string]$ApiSecret = "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx", 
...
try {
    ...
    Write-Host "Step 1: Obtaining access token from Cisco Umbrella..."
    $accessToken = Get-UmbrellaAccessToken -ApiKey $ApiKey -ApiSecret $ApiSecret
    ...
    Write-Host "Step 2: Fetching logs from Cisco Umbrella..."
    ...
    if ($logs -and $logs.Count -gt 0) {
        ...
        Write-Host "Step 3: Sending logs to Huntress in batches..."
        ...
        Write-Host "================================================"
        Write-Host "Log shipping completed successfully!"
        Write-Host "Events sent to Huntress: $successCount"
        ...
        Write-Host "1. No activity in the last 10 minutes"
        Write-Host "2. API permissions need adjustment"
        Write-Host "3. Different log types may be available"
    }
}
catch {
    ...
    Write-Host "1. API Key/Secret incorrect - Check credentials in Umbrella dashboard"
    Write-Host "2. Network issues - Check firewall/proxy settings"
    Write-Host "3. Huntress HEC endpoint/token incorrect - Verify with Huntress"
    exit 1
}
